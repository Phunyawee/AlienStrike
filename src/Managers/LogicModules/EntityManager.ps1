...
function New-Sin ([string]$name, [float]$x = 210, [float]$y = -150) {
    switch ($name.ToUpper()) {
        # --- ศัตรูทั่วไป ---
        "MINION"     { return [Enemy]::new($x, $y, 3, [System.Drawing.Color]::Lime) } # คลาส Enemy เดิม
        "WATCHER"    { return [Watcher]::new($x, $y, $x, 150, "Minion") }
        ...
        "REAL PRIDE"  { return [RealPride]::new($x, $y, $Script:player) }
